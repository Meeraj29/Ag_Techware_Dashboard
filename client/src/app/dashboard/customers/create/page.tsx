...
type Tab = 'Overview' | 'Address' | 'Contacts' | 'KYC & Documents' | 'Credit Control Details';

const TABS: Tab[] = ['Overview', 'Address', 'Contacts', 'KYC & Documents', 'Credit Control Details'];

function SelectField({ label, id, required, placeholder }: { label: string; id: string; required?: boolean; placeholder: string }) {
  return (
...
function InputField({ label, id, required, placeholder }: { label: string; id: string; required?: boolean; placeholder: string }) {
  return (
    <div className="flex flex-col gap-1.5">
      <label htmlFor={id} className="text-[13px] font-semibold text-gray-700">
        {label}{required && <span className="text-red-500 ml-0.5">*</span>}
...
function ToggleField({ label, id, defaultChecked }: { label: string; id: string; defaultChecked?: boolean }) {
...
export default function CreateCustomerPage() {
  ...
  const tabOrder: Tab[] = ['Overview', 'Address', 'Contacts', 'KYC & Documents', 'Credit Control Details'];

  const goToNextTab = () => {
    const currentIndex = tabOrder.indexOf(activeTab);
    const nextIndex = currentIndex < tabOrder.length - 1 ? currentIndex + 1 : currentIndex;
    ...
    <div className="min-h-full bg-gray-50/50">
      ...
      <div className="flex items-center justify-between px-6 py-4 bg-white border-b border-gray-200">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-[18px] font-bold text-gray-800">Add Customer Details</h1>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={() => router.back()}
          ...
          <div className="flex gap-0 min-w-max">
            {TABS.map((tab) => (
              <button
                ...
                className={`px-5 py-3 text-[14px] font-medium transition-all whitespace-nowrap border-b-2 ${activeTab === tab
        ...
        {activeTab === 'Overview' && (
          <div className="bg-white rounded-[16px] border border-gray-200 p-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-5">
              <InputField label="Company Name" id="company-name" required placeholder="Name" />
              <InputField label="Party Shortcode" id="party-shortcode" required placeholder="Party Shortcode" />
              <InputField label="Company CIN" id="company-cin" placeholder="Enter CIN" />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-5">
              <SelectField label="Party Type" id="party-type" required placeholder="Select" />
              <SelectField label="Currency" id="currency" required placeholder="Currency" />
              <SelectField label="Email (semi-colon to separate)" id="email" placeholder="Email (comma-separated)" />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
              <SelectField label="Import/Export" id="import-export" required placeholder="Import/Export" />
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-x-8 gap-y-4">
              ...
              <ToggleField label="Group Companies" id="group-companies" defaultChecked={true} />
              <ToggleField label="Billing Party" id="billing-party" defaultChecked={true} />
            </div>
          </div>
        )}
