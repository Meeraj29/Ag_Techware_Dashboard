...
export const vendorFormSlice = createSlice({
  ...
  initialState,
  reducers: {
    updateFormField: (state, action: PayloadAction<{ field: keyof VendorFormState; value: any }>) => {
      // @ts-ignore
    ...
    updateContact: (state, action: PayloadAction<{ id: string; field: keyof VendorContact; value: string }>) => {
...
export const { updateFormField, addContact, updateContact, removeContact, resetForm } = vendorFormSlice.actions;
