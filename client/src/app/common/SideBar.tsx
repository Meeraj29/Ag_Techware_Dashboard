			...
			<div className="flex-1 overflow-y-auto py-2 overflow-x-hidden scrollbar-hide">
				{sidebarData.map((section, idx) => (
					<div key={idx} className="mb-6">
						<h3
							className={`mb-3 px-6 text-[11px] font-bold uppercase tracking-wider text-gray-500 whitespace-nowrap ${isCollapsed ? "lg:hidden" : ""}`}
						>
							{section.title}
						</h3>
						{isCollapsed && (
								...
								return (
									<li key={item.name}>
										...
										>
											...
											/>
											<span
												className={`whitespace-nowrap text-sm ${isCollapsed ? "lg:hidden" : ""}`}
											>
												{item.name}
											</span>
										</Link>
