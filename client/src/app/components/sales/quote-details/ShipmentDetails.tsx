...
export default function ShipmentDetails({ quote }: { quote: SalesQuotation }) {
  const details = [
    ...
    { label: "Cargo Specs", value: `${quote.grossWeight.toLocaleString()} kg | ${quote.volume} CBM` },
    ...
    <div className="bg-white rounded-xl p-6  border border-gray-100 mb-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {details.map((item, index) => (
          <div key={index} className="bg-[#F8F9FA] h-32 p-8 rounded-lg flex flex-col justify-center">
            <span className="text-sm text-[#0000008A] font-medium mb-1">{item.label}</span>
            <span className="text-base font-medium text-black mt-4">{item.value}</span>
          </div>
        ))}
      </div>
    </div>
