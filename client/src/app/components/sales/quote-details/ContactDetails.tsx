...
export default function ContactDetails({ contact }: { contact?: ContactDetailsType }) {
    ...
    <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mb-6">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between bg-[#F8F9FA] p-3 rounded-lg">
          <span className="text-xs font-medium text-gray-900 truncate">{contact.email}</span>
          <Mail className="h-4 w-4 text-gray-400 shrink-0 ml-2" />
        </div>

        <div className="flex items-center justify-between bg-[#F8F9FA] p-3 rounded-lg">
          <span className="text-xs font-medium text-gray-900">{contact.phone}</span>
        ...
        <div className="flex items-start justify-between bg-[#F8F9FA] p-3 rounded-lg">
          <div>
            <span className="block text-[10px] text-gray-500 font-semibold mb-0.5">Billing Address</span>
            <span className="text-xs font-medium text-gray-900">{contact.address}</span>
          </div>
