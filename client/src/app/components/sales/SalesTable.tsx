...
import { MoreVertical, ChevronLeft, ChevronRight, Sailboat } from "lucide-react";

export default function SalesTable() {
  const { quotations, filters } = useSelector((state: RootState) => state.sales);

  const getStatusColor = (status: string) => {
    switch (status) {
      case "Approved": return "text-[#059669]";
      case "Pending": return "text-[#D97706]";
      case "Rejected": return "text-[#C10209]";
      case "Draft": return "text-gray-500";
      case "Converted To Job": return "text-[#3525CD]";
    ...
    <div className="flex flex-col flex-1 px-4 pb-4">
        ...
        <table className="w-full text-left border-collapse min-w-[1000px]">
          <thead>
            <tr className="bg-[#F4F4F4]">
              <th className="py-4 px-6 text-base font-medium text-[#000000CC] uppercase tracking-wider">Quote ID</th>
              <th className="py-4 px-6 text-base font-medium text-[#000000CC] uppercase tracking-wider">Customer</th>
              <th className="py-4 px-6 text-base font-medium text-[#000000CC] uppercase tracking-wider">Category</th>
              <th className="py-4 px-6 text-base font-medium text-[#000000CC] uppercase tracking-wider">Destination</th>
              <th className="py-4 px-6 text-base font-medium text-[#000000CC] uppercase tracking-wider">Est Value</th>
              <th className="py-4 px-6 text-base font-medium text-[#000000CC] uppercase tracking-wider">Status</th>
              <th className="py-4 px-6 text-base font-medium text-[#000000CC] uppercase tracking-wider text-center">Actions</th>
            </tr>
          </thead>
          <tbody>
            ...
            {filteredQuotations.map((quote, index) => {
              ...
              const estValue = quote.productDetails.reduce((sum, item) => sum + item.sellTotal, 0);
              ...
              return (
                ...
                >
                  ...
                  <td className="py-4 px-6">
                    <span className={`inline-flex items-center gap-2 px-3 py-2 rounded-full text-xs font-medium ${quote.tradeType === 'Import'
                  ...
                  <td className="py-4 px-6 text-base font-medium text-black">
                    {quote.localCurrency === 'INR' ? '₹' : quote.localCurrency === 'USD' ? '$' : '€'}
                    {estValue.toLocaleString()}
                  </td>
                  <td className="py-4 px-6 text-sm font-semibold">
                    <div className="flex items-center gap-2">
                      <div className={`w-2 h-2 rounded-full ${getStatusColor(quote.status).replace('text-', 'bg-')}`} />
      ...
      <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100 mt-auto">
        <span className="text-sm font-semibold text-gray-500">
          Results: {filteredQuotations.length.toString().padStart(2, '0')} Out Of {quotations.length.toString().padStart(2, '0')}
        </span>
        <div className="flex items-center gap-2">
