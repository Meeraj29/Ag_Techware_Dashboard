...
export function LedgerTable() {
  const customers = useSelector((state: any) => state.creditControl?.customers || []);
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'Over Limit': return 'bg-[#BA1A1A]/20 text-[#BA1A1A]';
      case 'Near Limit': return 'bg-[#F59E0B]/20 text-[#F59E0B]';
      case 'Within Limit': return 'bg-[#059669]/20 text-[#059669]';
  ...
  const filteredCustomers = customers.filter((c: Customer) => {
    ...
    const matchesCredit = selectedCredit === 'All' || c.status === selectedCredit;
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="relative">
            ...
            {openDropdown === 'credit' && (
              <div className="absolute top-full left-0 mt-1 bg-white border border-[#EBEBEB] rounded-[12px] shadow-lg py-1.5 min-w-[180px] z-50">
                {['All', 'Within Limit', 'Near Limit', 'Over Limit'].map((option) => (
                  <div
                    key={option}
                    onClick={() => {
          ...
          <div className="relative">
            ...
            >
              ...
              <span className="text-black font-semibold">{selectedPayment}</span>
              <ChevronDown className="w-4 h-4 text-black/70" />
            </button>
          ...
          <button className="text-[14px] font-medium text-[#04468D] hover:text-gray-800 whitespace-nowrap px-3 py-2.5 flex items-center gap-1.5 border border-[#D8D8D8] rounded-[12px]">
            Advanced Filters<Image width={14} height={14} src="/Filter.svg" alt="" />
          </button>
        </div>
      </div>
        ...
        <tr key={c.id} className="hover:bg-gray-50 transition-colors">
          ...
          <td
            className={`px-7 py-5 font-medium whitespace-nowrap ${
              c.overdue > 0 ? "text-[#BA1A1A] text-[16px]" : "text-black text-[16px]"
            }`}
          >
