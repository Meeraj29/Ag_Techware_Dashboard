...
export function TopDefaulters() {
  const defaulters = useSelector((state: any) => state.creditControl?.topDefaulters || []);
          ...
          <div key={d.id} className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[14px] text-black/80">{d.daysOverdue} Days Overdue</p>
              </div>
            </div>
            <p className="font-medium text-[#BA1A1A] text-[18px]">₹{d.overdueAmount.toLocaleString('en-US')}</p>
          </div>
        ))}
      </div>
