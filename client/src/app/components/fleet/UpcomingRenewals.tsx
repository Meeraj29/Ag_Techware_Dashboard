...
export default function UpcomingRenewals() {
  ...
  return (
    <div className="rounded-[24px] border border-gray-100 bg-white p-6 shadow-sm mt-6">
      <h3 className="text-sm font-bold text-gray-900 mb-4">Upcoming Renewals</h3>
      ...
      <div className="space-y-3">
        {renewalItems.map((item) => (
          ...
          >
            <div className="flex items-start gap-3">
              <div className={`mt-0.5 ${item.isUrgent ? "text-[#d35400]" : "text-[#0052cc]"}`}>
                {item.type === "insurance" ? (
              ...
              <div>
                ...
                <p className="text-[10px] text-gray-500 mt-0.5">{item.expiresIn}</p>
              </div>
            </div>
        ...
        {renewalItems.length === 0 && (
          <div className="py-4 text-center text-xs text-gray-500">No upcoming renewals.</div>
        )}
      </div>
    </div>
  );
