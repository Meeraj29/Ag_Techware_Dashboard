...
export default function DriverTable() {
  const { driverRecords, searchQuery, typeFilter, statusFilter } = useSelector((state: RootState) => state.fleet);
      ...
      <div className="min-w-[1000px]">
        ...
        <div className="divide-y divide-gray-100">
          {filteredRecords.map((record) => (
            ...
            >
              <div>
                ...
                <p className="text-[10px] text-gray-400 font-medium mt-0.5">ID: {record.driverId}</p>
              </div>
              <div className="text-gray-700 font-medium">{record.contact}</div>
              <div className={`font-semibold ${record.assignedVehicle === "Unassigned" ? "text-gray-400" : "text-blue-600 hover:underline cursor-pointer"}`}>
                {record.assignedVehicle}
              </div>
              <div>
                <p className={`font-semibold ${record.isLicenseOverdue ? "text-red-500" : "text-gray-950"}`}>
                  {record.licenseExpiry}
                </p>
                <p className="text-[9px] text-gray-400 font-semibold mt-0.5">{record.licenseExpiryDetail}</p>
              </div>
              <div>
                <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 font-bold text-[9px] tracking-wide ${
                  record.status === "Assigned" ? "bg-blue-50 text-blue-600" :
                  record.status === "Available" ? "bg-emerald-50 text-emerald-600" :
          ...
          {filteredRecords.length === 0 && (
            <div className="py-12 text-center text-gray-500">No drivers found matching your filters.</div>
          )}
        </div>
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between text-xs font-medium text-gray-500 px-2">
          ...
          <div>Showing 1-{filteredRecords.length} Of 142 Documents & Compliance</div>
          <div className="inline-flex items-center gap-1">
            <button className="rounded-lg border border-gray-200 p-2 text-gray-400 hover:bg-gray-50"><ChevronLeft className="h-4 w-4" /></button>
            <span className="rounded-lg border border-gray-200 px-3 py-2 text-gray-700 bg-gray-50 font-bold">1</span>
            <button className="rounded-lg border border-gray-200 p-2 text-gray-400 hover:bg-gray-50"><ChevronRight className="h-4 w-4" /></button>
          </div>
        </div>
      </div>
    </div>
