...
export default function TripTable() {
	const { trips, searchQuery, typeFilter, statusFilter } = useSelector(
	...
	const filteredTrips = trips.filter((trip) => {
		...
		const matchesType =
			typeFilter === "All" ||
			trip.vehicle.toLowerCase().includes(typeFilter.toLowerCase());
		const matchesStatus =
				...
				<div className="mt-2 space-y-2">
					{filteredTrips.map((trip, index) => (
						...
						>
							...
							<div className="text-gray-600 whitespace-pre-line leading-tight text-xs">
								{trip.departure}
							</div>
							<div className="text-gray-600">{trip.distance}</div>
							<div className="flex items-center justify-end gap-2">
								<span
									className={`rounded-full px-3 py-1 text-xs font-semibold ${trip.status === "Active" ? "bg-sky-100 text-sky-700" : trip.status === "Delayed" ? "bg-red-100 text-red-700" : trip.status === "Planned" ? "bg-amber-100 text-amber-700" : "bg-emerald-100 text-emerald-700"}`}
								>
									{trip.status}
								</span>
								<button className="rounded-full p-2 text-gray-500 hover:bg-gray-100 transition">
