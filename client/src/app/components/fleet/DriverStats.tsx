        ...
        return (
          <div 
            ...
            className={`relative flex justify-between rounded-2xl p-6 h-36 border ${
              card.isAlert ? "bg-[#fff8f8] border-[#ffe8e8]" : "bg-[#f5f6f7] border-transparent"
            }`}
          >
            <div className="flex flex-col justify-between">
              ...
              <p className={`text-[10px] font-medium ${card.isAlert ? "text-red-500" : "text-gray-500"}`}>
                {card.subtext}
              </p>
            </div>
            <div className="flex flex-col items-end">
              <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${
                card.isAlert ? "bg-[#ffe8e8] text-red-500" : "bg-[#e3e5e8] text-gray-800"
              }`}>
                <Icon className={`h-4 w-4 ${card.isStar ? "text-amber-500 fill-amber-500" : ""}`} />
              </div>
            </div>
          </div>
        );
