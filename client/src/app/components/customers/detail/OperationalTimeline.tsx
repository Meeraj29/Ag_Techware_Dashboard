  ...
  return (
    <div className="bg-white rounded-[20px] border border-gray-200 p-6 flex-1">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-[18px] font-bold text-gray-800">Operational Timeline</h2>
        ...
        <table className="w-full text-left text-[14px]">
          <thead>
            <tr className="bg-[#F8F9FA] text-gray-500 border-b border-gray-100">
              <th className="py-4 px-4 font-medium rounded-l-[12px]">Date & Time</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {detail.timeline.map((entry) => (
              <tr key={entry.id} className="hover:bg-gray-50/50 transition">
                <td className="py-4 px-4 text-gray-800 font-medium">{entry.dateTime}</td>
                ...
                <td className="py-4 px-4">
                  <div className="flex items-center gap-2">
                    <div className={`w-1.5 h-1.5 rounded-full ${getStatusColor(entry.status).replace('text-', 'bg-')}`} />
                    <span className={`font-semibold ${getStatusColor(entry.status)}`}>{entry.status}</span>
                  </div>
                </td>
              </tr>
            ))}
