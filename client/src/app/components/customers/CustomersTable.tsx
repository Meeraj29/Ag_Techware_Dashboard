					...
					<tbody className="divide-y divide-gray-100">
						{paginatedCustomers.map((cust) => (
							...
							>
								...
								<td className="px-6 py-4">
									<span
										className={`text-xs font-semibold px-3 py-1 rounded-xl ${getTypeBadgeStyles(cust.type)}`}
									>
										{cust.type}
									</span>
								</td>
								...
								<td className="px-6 py-4">
									<div className="flex items-center gap-1.5">
										<span
											className={`w-2.5 h-2.5 rounded-full ${getStatusDotColor(cust.status)}`}
										/>
			...
			<div className="p-6 flex flex-col sm:flex-row gap-4 items-center justify-between border-t border-gray-100 text-sm text-gray-500">
				<div>
					...
					<span className="font-semibold text-gray-700">
						{paginatedCustomers.length.toString().padStart(2, "0")}
					</span>{" "}
					Out Of{" "}
					<span className="font-semibold text-gray-700">
