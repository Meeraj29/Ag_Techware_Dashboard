...
export default function TrackingDetails() {
	const { id } = useParams();
	const foundJob = useSelector((state: RootState) =>
		state.tracking.jobs.find((j) => j.id === id),
	);
	...
	return (
		<div className="w-full space-y-4 sm:space-y-6 p-4">
			...
			<div className="bg-[#FFFBEB] rounded-xl p-4 sm:p-5 flex items-start gap-4">
				...
				<div>
					<div className="text-xl font-semibold text-[#78350F] mb-1">
						Potential Port Congestion Alert
					</div>
					<div className="text-md font-regular text-[#92400E]">
						Minimal impact expected for current schedule, but monitoring local
						labor activity in Rotterdam.
					</div>
				</div>
			</div>
			...
			<div className="grid grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4 bg-white rounded-xl p-4 sm:p-4 border border-gray-150">
				<div className="bg-[#F4F4F4] border-1 border-[#EDEDED] rounded-xl p-3 flex flex-col justify-center">
					<div className="text-[14px] font-medium text-[#464555] mb-1">
						Origin
					</div>
					<div className="text-lg font-semibold text-[#000000]">
						{details.origin}
					</div>
					<div className="text-[14px] font-regular text-[#464555] mt-1">
						{details.originSub}
					</div>
				</div>
				<div className="bg-[#F4F4F4] border-1 border-[#EDEDED] rounded-xl p-3 flex flex-col justify-center">
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-4">
				...
				<div className="lg:col-span-2 bg-white rounded-2xl p-4 sm:p-4 shadow-sm border border-gray-100">
					<h2 className="text-[16px] sm:text-[18px] font-semibold text-[#000000] mb-6 sm:mb-8">
								...
								{event.status === "current" ? (
									<div className="bg-[#04489033] rounded-xl p-3 sm:p-4 ml-2 -mt-2">
										<div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-2 gap-1 sm:gap-0">
											<div className="font-semibold text-[#044890] text-[16px]">
												{event.title}
											</div>
											<div className="text-[12px] font-semibold text-[#044890]">
												{event.date}
											</div>
										</div>
										{event.extraInfo && (
											<>
												<p className="text-[14px] text-[#1B1B24] font-regular mb-3">
													{event.extraInfo.description}
												</p>
												<div className="flex flex-wrap items-center gap-2 sm:gap-4 text-[13px] font-medium text-[#464555]">
													<div>
														...
														<span className="font-semibold text-[#1B1B24]">
															{event.extraInfo.speed}
														</span>
													</div>
													<div>
														Heading:{" "}
														<span className="font-semibold text-[#1B1B24]">
															{event.extraInfo.heading}
														</span>
													</div>
												</div>
											</>
									...
									<div className="ml-2 flex flex-col sm:flex-row sm:justify-between sm:items-start gap-1 sm:gap-0">
										<div>
											<div
												className={`font-semibold text-[16px] ${event.status === "pending" ? "text-[#505050]" : "text-[#000000]"}`}
											>
												{event.title}
											</div>
											{event.subtitle && (
												<div className="text-[14px] text-[#464555] font-regular mt-1">
													{event.subtitle}
												</div>
											)}
										</div>
										<div
											className={`text-[12px] font-medium ${event.status === "pending" ? "text-[#505050]" : "text-[#464555]"}`}
										>
											{event.date}
										</div>
									</div>
				...
				<div className="bg-white rounded-2xl p-4 sm:p-4 shadow-sm border border-gray-100 flex flex-col">
					<h2 className="text-[16px] sm:text-[18px] font-semibold text-[#1B1B24] mb-4 sm:mb-4">
						Pending Tracking Actions
					</h2>
					<div className="space-y-3 flex-1">
						{details.pendingActions.map((action, i) => (
							<div
								key={i}
								className="flex flex-col sm:flex-row sm:justify-between sm:items-center bg-[#F4F4F4] rounded-xl p-4 gap-3 sm:gap-0"
							>
								<div>
									<div className="text-[14px] font-semibold text-[#1B1B24]">
										{action.title}
									</div>
									<div className="text-[12px] font-regular text-[#464555] mt-1">
										{action.subtitle}
									</div>
								</div>
				...
				<div className="lg:col-span-3 bg-white rounded-2xl p-4 sm:p-4 shadow-sm border border-gray-100">
					<div className="flex justify-between items-center mb-4 sm:mb-4">
						<h2 className="text-[20px] sm:text-[18px] font-medium text-[#000000]">
							Internal Notes
						</h2>
						<button className="text-[#3525CD] text-sm font-medium hover:underline">
							Add Note
						</button>
					</div>
					<div className="space-y-2">
						{details.internalNotes.map((note, i) => (
							<div
								key={i}
								className="bg-[#F2F2F2] rounded-xl p-4 flex flex-col sm:flex-row justify-between gap-4"
							>
								<p className="text-sm font-medium text-[#000000] max-w-2xl">
									{note.text}
								</p>
								<div className="text-[12px] font-regular text-[#000000B2] shrink-0 self-end sm:self-auto">
									{note.author}
								</div>
							</div>
						))}
				...
				{/* ACTIVITY LOG (Span 3) */}
				<div className="lg:col-span-3 bg-white rounded-2xl p-4 sm:p-4 shadow-sm border border-gray-100">
					<h2 className="text-[16px] sm:text-[18px] font-semibold text-[#000000] mb-4 sm:mb-4">
						Activity Log
					</h2>
						...
						{details.activityLogs.map((log, i) => (
							<div key={i} className="relative flex items-start gap-4">
								...
								<div>
									...
									<div className="text-[12px] font-regular text-[#000000B2] mt-1">
										{log.date}
									</div>
								</div>
							</div>
