...
import { setPrimaryTab, setSecondaryTab, PrimaryTabType, SecondaryTabType } from "../../redux/features/clearance/clearanceSlice";
...
export default function ClearanceTableContainer() {
  ...
  const { primaryTab, secondaryTab, jobs } = useSelector((state: RootState) => state.clearance);

  const pendingCount = jobs.filter(j => j.status === "Under Review" || j.status === "Pending").length;
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col mb-8 mt-4">
      ...
      <div className="px-6 pt-6 pb-2 border-b border-gray-100 bg-[#F8F9FA] flex gap-2">
        <div className="bg-[#EEF1F3] flex items-center justify-center gap-3 p-2 rounded-lg">
          ...
          >
            Pending Reviews
            <span className={`text-xs px-1.5 py-0.5 rounded-full font-bold ${primaryTab === "Pending Reviews" ? "bg-white text-[#075FB7]" : "bg-gray-200 text-gray-500"}`}>{pendingCount}</span>
          </Button>
          <Button
          ...
          >
            Alerts
            <span className={`text-xs px-1.5 py-0.5 rounded-full font-bold ${primaryTab === "Alerts" ? "bg-white text-[#075FB7]" : "bg-gray-200 text-gray-500"}`}>{alertCount}</span>
          </Button>
          <Button
          ...
          >
            All
            <span className={`text-xs px-1.5 py-0.5 rounded-full font-bold ${primaryTab === "All" ? "bg-white text-[#075FB7]" : "bg-gray-200 text-gray-500"}`}>{allCount}</span>
          </Button>
        </div>
      </div>
