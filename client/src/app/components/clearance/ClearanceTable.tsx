...
export default function ClearanceTable() {
  ...
  const { jobs, primaryTab, secondaryTab } = useSelector((state: RootState) => state.clearance);
  ...
  const getStatusBadge = (status: ClearanceJob["status"]) => {
    switch (status) {
      case "Under Review":
        return <span className="flex items-center gap-1.5 text-xs font-medium text-[#F59E0B]"><span className="w-1.5 h-1.5 rounded-full bg-[#F59E0B]"></span>Under Review</span>;
      case "Pending":
        return <span className="flex items-center gap-1.5 text-xs font-medium text-[#2563EB]"><span className="w-1.5 h-1.5 rounded-full bg-[#2563EB]"></span>Pending</span>;
      case "Hold":
        return <span className="flex items-center gap-1.5 text-xs font-medium text-[#B91C1C]"><span className="w-1.5 h-1.5 rounded-full bg-[#B91C1C]"></span>Hold</span>;
      case "Approved":
        return <span className="flex items-center gap-1.5 text-xs font-medium text-[#047857]"><span className="w-1.5 h-1.5 rounded-full bg-[#047857]"></span>Approved</span>;
      default:
        return null;
    }
  };
  ...
  const getSlaTimerStyle = (timer: string, status: string) => {
    ...
    if (timer.startsWith("-")) return "text-red-500 font-medium flex items-center gap-1";
    if (status === "Under Review") return "text-red-500 font-medium flex items-center gap-1"; // Matching screenshot color
      ...
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="bg-[#F8F9FA] border-y border-gray-100">
            <th className="px-6 py-4 text-xs font-bold text-gray-900">Job ID</th>
            <th className="px-6 py-4 text-xs font-bold text-gray-900">Customer</th>
            ...
            {primaryTab !== "Alerts" && <th className="px-6 py-4 text-xs font-bold text-gray-900">Stage</th>}
            <th className="px-6 py-4 text-xs font-bold text-gray-900">Status</th>
            {primaryTab === "Alerts" && <th className="px-6 py-4 text-xs font-bold text-gray-900">Alert Type</th>}
            <th className="px-6 py-4 text-xs font-bold text-gray-900">Sla Timer</th>
            <th className="px-6 py-4 text-xs font-bold text-gray-900 text-center">Actions</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-gray-100 bg-white">
          {filteredJobs.map((job, index) => (
            ...
            >
              ...
              <td className="px-6 py-4">
                <span className={`text-xs ${getSlaTimerStyle(job.slaTimer, job.status)}`}>
                  {(job.slaTimer !== "Expired" && job.slaTimer !== "Completed" && job.slaTimer !== "-------") && (
