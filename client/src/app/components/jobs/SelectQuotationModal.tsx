...
export default function SelectQuotationModal({ isOpen, onClose }: SelectQuotationModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl ">
        ...
        <div className="p-6 pb-4">
          <h2 className="text-xl font-medium text-center text-gray-900">Select Quotation</h2>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-6 space-y-3 scrollbar-hide">
          {filteredQuotations.map((quote) => {
            ...
            const origin = quote.placeOfCarrierReceipt.split(' ')[0] || "Origin";
            const destination = quote.placeOfCarrierDelivery.split(' ')[0] || "Destination";

            return (
              <div
                key={quote.id}
                onClick={() => setSelectedQuoteId(quote.id)}
                className={`flex items-center  justify-between p-4 rounded-xl border-2 cursor-pointer transition-colors ${isSelected ? 'border-[#075FB7]' : 'border-gray-100 hover:border-gray-200'
                  }`}
              >
                <div className="w-1/4">
                ...
                <div className="w-1/4 flex justify-center">
                  <span className={`inline-flex items-center gap-1.5 px-3 py-2 rounded-full text-xs font-medium ${quote.tradeType === 'Import'
        ...
        <div className="p-6 border-t border-gray-100 flex items-center justify-end gap-3">
          <Button variant="outline" onClick={onClose} className="px-6 rounded-lg text-xs cursor-pointer">
            Cancel
          </Button>
          <Button
            variant="default"
