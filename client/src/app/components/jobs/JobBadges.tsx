...
export function CategoryBadge({ category }: { category: string }) {
  ...
  return (
    <span className={`px-2.5 py-2 text-xs font-bold rounded-md ${bgColor} ${textColor}`}>
      {category}
    </span>
  );
}
...
export function UrgencyBadge({ urgency }: { urgency: string }) {
  ...
  return (
    <span className={`px-4 py-1.5 text-xs font-bold rounded-md border ${bgColor} ${textColor} ${borderColor}`}>
      {urgency}
    </span>
  );
}

export function ActionButton({ label, type }: { label: string, type: 'primary' | 'secondary' | 'default' }) {
  const baseStyle = "px-4 py-1.5 text-xs font-semibold rounded-md border transition-colors";
  if (type === "primary") {
    return (
      <button className={`${baseStyle} bg-[#075FB7] text-white border-[#075FB7] hover:bg-[#064a8f]`}>
        {label}
      </button>
    );
  }
  if (type === "secondary") {
    return (
      <button className={`${baseStyle} bg-[#E8F0FE] text-[#075FB7] border-transparent hover:bg-[#d2e3fc]`}>
        {label}
      </button>
    );
  }
  return (
    <button className={`${baseStyle} bg-white text-gray-700 border-gray-300 hover:bg-gray-50`}>
      {label}
    </button>
  );
}
