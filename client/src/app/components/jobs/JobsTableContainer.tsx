...
export default function JobsTableContainer() {
	const [primaryTab, setPrimaryTab] = useState<"All" | "Import" | "Export">(
		"All",
	);
	const [secondaryTab, setSecondaryTab] = useState<
		"All jobs" | "Action Required"
	>("All jobs");
	...
	return (
		<div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col mt-4">
			...
			<div className="px-6 pt-6 pb-2 border-b border-gray-100 bg-[#F8F9FA] flex gap-2">
				<div className="bg-[#EEF1F3] flex items-center justify-center gap-3 p-2">
					...
					>
						All
						<span
							className={`text-xs px-1.5 py-0.5 rounded-full ${primaryTab === "All" ? "bg-white text-[#075FB7]" : "bg-gray-200 text-gray-500"}`}
						>
							15
						</span>
					</button>
					...
					>
						Import
						<span
							className={`text-xs px-1.5 py-0.5 rounded-full ${primaryTab === "Import" ? "bg-white text-[#075FB7]" : "bg-gray-200 text-gray-500"}`}
						>
							10
						</span>
					</button>
					...
					>
						Export
						<span
							className={`text-xs px-1.5 py-0.5 rounded-full ${primaryTab === "Export" ? "bg-white text-[#075FB7]" : "bg-gray-200 text-gray-500"}`}
						>
							16
						</span>
					</button>
			...
			<div className="px-6 flex gap-6 border-b border-gray-200 pt-4">
				...
				>
					All jobs
					<span className="text-[10px] px-1.5 py-0.5 rounded-full bg-[#CBCBCB] text-black">
						15
					</span>
				</button>
				<button
