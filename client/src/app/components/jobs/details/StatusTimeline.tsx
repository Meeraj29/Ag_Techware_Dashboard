...
export default function StatusTimeline({ timeline }: { timeline: any[] }) {
  const getIcon = (stepName: string, status: string) => {
    ...
    switch (stepName) {
      case "In Transit": return <Ship className="w-5 h-5 text-white" />;
      case "Port Arrival": return <Anchor className="w-5 h-5 text-white" />;
      case "Delivered": return <Warehouse className="w-5 h-5 text-white" />;
      default: return <Circle className="w-5 h-5 text-white" />;
    }
  };
  ...
  let blueLineStyle = { left: `calc(${startOffsetPercent}% - 28px)`, width: "0%" };
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 overflow-x-auto">
      <h3 className="text-base font-medium text-gray-900 mb-8">Status Timeline</h3>
          ...
          return (
            <div key={index} className="flex flex-col items-center relative flex-1">
              ...
              {index !== timeline.length - 1 && (
                <div className={`absolute top-[81px] right-0 translate-x-1/2 -translate-y-1/2 z-10 ${isNextLineBlue ? 'text-[#075FB7]' : 'text-gray-300'}`}>
              ...
              {/* Top Icon Box */}
              <div className={`w-14 h-14 rounded-md flex items-center justify-center mb-5 relative z-10 ${isPastOrCurrent ? 'bg-gradiate' : 'bg-[#677180]'
                }`}>
                {getIcon(item.step, item.status)}
              </div>

              {/* The Dot on the line */}
              <div className={`w-[10px] h-[10px] rounded-full relative z-10 mb-3 ${isPastOrCurrent ? 'bg-[#075FB7]' : 'bg-[#E5E7EB]'
                }`}></div>
              ...
              <div className="text-center flex flex-col items-center w-full px-2">
                ...
                <p className={`text-[10px] mt-1 ${isCurrent ? 'text-[#04458B] font-semibold' : 'text-gray-500'}`}>
                  {item.date}
                </p>
              </div>
            </div>
