...
export default function ActionsRequiredSidebar({ actions }: { actions: any[] }) {
    ...
    <div className="bg-[#0863BC] rounded-2xl p-5  shadow-md">
      ...
      <div className="flex flex-col gap-3">
        {actions.map(action => {
          ...
          return (
            <div key={action.id} className={`p-4 rounded-l-xl ${style.bg} ${style.border}`}>
              <div className="flex justify-between items-center gap-4">
                <div className="flex-1 pr-2">
                  <h4 className={`text-sm font-bold mb-1 ${style.title}`}>{action.title}</h4>
                  <p className="text-sm text-gray-700 leading-snug">{action.description}</p>
                </div>
