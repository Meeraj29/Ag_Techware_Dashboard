...
export default function JobsTable({
	...
	activeTab: "All jobs" | "Action Required";
}) {
				...
				<tbody>
					{jobs.map((job, index) => (
						<tr
							key={index}
							onClick={() =>
								router.push(`/dashboard/jobs/${encodeURIComponent(job._id)}`)
							}
						...
						>
							...
							<td className="py-4 px-6 whitespace-nowrap">
								{isActionRequired ? (
									<p
										className={`text-sm font-medium ${job.urgency === "Critical" ? "text-[#E44444]" : job.urgency === "Medium" ? "text-[#D97706]" : "text-[#000000B2]"}`}
									>
										{job.issuesType}
									</p>
								) : (
							...
							<td className="py-4 px-6 whitespace-nowrap">
								{isActionRequired ? (
									...
									<div>
										...
										<p className="text-xs text-[#000000B2] mt-0.5">
											{job.route.type}
										</p>
									</div>
								)}
								...
								<>
									...
									<td className="py-4 px-6 whitespace-nowrap">
										...
										<div className="flex items-center gap-1 mt-0.5">
											<div
												className={`w-1.5 h-1.5 rounded-full ${job.eta.status.includes("Delay") || job.eta.status.includes("Stalled") ? (job.eta.status.includes("Stalled") ? "bg-[#BA1A1A]" : "bg-[#D97706]") : "bg-[#059669]"}`}
											></div>
											<p
												className={`text-xs ${job.eta.status.includes("Delay") || job.eta.status.includes("Stalled") ? (job.eta.status.includes("Stalled") ? "text-[#BA1A1A]" : "text-[#D97706]") : "text-[#059669]"}`}
											>
												{job.eta.status}
											</p>
										</div>
							...
							<td className="py-4 px-6 whitespace-nowrap">
								{isActionRequired ? (
									<ActionButton
										label={
											job.issuesType.includes("Missing") ||
											job.issuesType.includes("Customs")
												? "Upload"
												: job.issuesType.includes("Damaged")
													? "Contact"
													: "Review"
										}
										type={
