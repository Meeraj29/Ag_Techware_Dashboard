...
export default function ValueAddedServicesForm({ data, onChange }: ValueAddedServicesFormProps) {
  const services = [
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-sm mb-6">
      <h2 className="text-base font-bold text-gray-900 mb-6">Value Added Services</h2>
      
      <div className="flex flex-wrap items-center gap-x-8 gap-y-4">
        {services.map((service) => (
          <label key={service.id} className="flex items-center gap-2.5 cursor-pointer">
            <input
              type="checkbox"
            ...
            />
            <span className="text-[13px] font-medium text-gray-700">{service.label}</span>
          </label>
        ))}
      </div>
    </div>
