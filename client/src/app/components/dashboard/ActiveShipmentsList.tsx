...
export default function ActiveShipmentsList() {
	const activeShipments = useAppSelector(
		(state) => state.dashboard.activeShipments,
	);
			...
			<div className="overflow-x-auto">
				<table className="w-full text-left text-sm whitespace-nowrap">
					<thead className="bg-gray-50 text-gray-600 font-bold border-b border-gray-100">
						<tr>
							<th className="py-3 px-5 text-[#000000CC] text-base font-medium">
								Job ID
							</th>
							<th className="py-3 px-5 text-[#000000CC] text-base font-medium">
								Customer
					...
					<tbody className="divide-y divide-gray-50 text-gray-800 font-medium">
						{activeShipments.map((shipment, idx) => (
							<tr key={idx} className="hover:bg-gray-50/50 transition-colors">
								<td className="py-4 px-5 text-[#3525CD] font-medium text-base">
									{shipment.id}
								</td>
								<td className="py-4 px-5 font-medium text-base">
									{shipment.customer}
								</td>
								<td className="py-4 px-5">
									<span
										className={`px-3 py-2 rounded-full text-xs font-medium flex items-center w-fit gap-1 ${
								...
								<td className="py-4 px-5 text-black text-base font-medium whitespace-normal min-w-[200px]">
									{shipment.origin}
								</td>
							</tr>
						))}
