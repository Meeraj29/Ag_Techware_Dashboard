      ...
      <div className="overflow-y-auto flex-1 p-5 space-y-4 max-h-[600px] scrollbar-hide">
        {alerts.map((alert, idx) => (
          <div key={idx} className="relative pl-4 border-l-2 border-gray-100">
            ...
            <div className={`absolute -left-px top-0 bottom-0 w-[2px] ${alert.type === 'Shipment Delayed' ? 'bg-[#BA1A1A]' :
              alert.type === 'Missing Documentation' ? 'bg-amber-400' :
                'bg-blue-400'
              }`}></div>

            <div className="flex justify-between items-start mb-1">
              <span className={`text-xs font-medium tracking-wider ${alert.type === 'Shipment Delayed' ? 'text-[#BA1A1A]' :
                alert.type === 'Missing Documentation' ? 'text-amber-500' :
                  'text-blue-400'
                }`}>{alert.type}</span>
              {alert.type === 'Shipment Delayed' && <Clock size={14} className="text-red-500" />}
              {alert.type === 'Missing Documentation' && <FileWarning size={14} className="text-amber-500" />}
            </div>

            <h3 className="text-base font-medium text-black mb-1">{alert.title}</h3>
