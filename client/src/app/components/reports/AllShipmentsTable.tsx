...
export function AllShipmentsTable() {
  const shipments = useSelector((state: { reports: ReportsState }) => state.reports.allShipments);
            ...
            {paginatedShipments.map((s, index) => {
              ...
              return (
                <tr key={`${s.id}-${s.route}-${index}`} className="hover:bg-gray-50 transition-colors">
                  ...
                  {/* Type */}
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold ${getTypeStyle(s.type)}`}>
