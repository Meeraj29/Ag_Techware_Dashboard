...
export function ShipmentTrends() {
  const trends = useSelector((state: { reports: ReportsState }) => state.reports.trends);
      ...
      <div className="flex-1 w-full min-h-0">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={trends} barGap={4}>
            ...
            />
            <Bar dataKey="lastMonth" fill="#C1D2FF" radius={[4, 4, 0, 0]} barSize={24} />
            <Bar dataKey="thisMonth" fill="#0047FF" radius={[4, 4, 0, 0]} barSize={24} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>
