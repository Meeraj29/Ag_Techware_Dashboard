...
export function IntelligenceInsights() {
  const insights = useSelector((state: { reports: ReportsState }) => state.reports.intelligenceInsights);
    ...
    <div className="bg-[#0A4B9F] rounded-[24px] p-6 h-full flex flex-col text-white shadow-sm">
      ...
      <div className="flex flex-col gap-3 flex-1 overflow-y-auto pr-1 custom-scrollbar">
        {insights.map((insight) => (
          <div key={insight.id} className="bg-white/10 rounded-xl p-4 backdrop-blur-sm border border-white/5">
            <h3 className="font-semibold text-[15px] mb-1.5">{insight.title}</h3>
