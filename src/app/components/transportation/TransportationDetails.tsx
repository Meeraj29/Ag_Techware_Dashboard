...
export default function TransportationDetails() {
	...
	const job = useSelector((state: RootState) =>
		state.transportation.jobs.find((j) => j.id === id),
	);
		...
		return (
			<div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
				...
				<p className="text-gray-500">
					The requested job could not be located in our records.
				</p>
				<Link
	...
	return (
		<div className="w-full space-y-4 sm:space-y-6">
			...
			<div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
				<div className="shrink-0">
					<div className="flex items-center gap-3 mb-2">
						<Link
							href="/dashboard/transportation"
							className="p-1.5 bg-white border border-gray-200 rounded-md hover:bg-gray-50 text-gray-500 shrink-0"
						>
							<ChevronLeft className="w-5 h-5" />
						</Link>
						<h1 className="text-lg sm:text-xl font-semibold text-[#000000] whitespace-nowrap">
							Job ID {job.jobId}
						</h1>
						<span
							className={`px-2 py-1 sm:px-3 text-[10px] sm:text-xs font-bold rounded-full whitespace-nowrap ${
								job.status === "In Transit"
									? "bg-[#3525CD4D] text-[#3525CD]"
									: job.status === "Delayed"
										? "bg-[#F59E0B33] text-[#F59E0B]"
										: job.status === "Completed"
											? "bg-[#05966933] text-[#059669]"
					...
					<div className="flex flex-wrap items-center gap-2 text-xs sm:text-sm text-[#000000] sm:pl-11">
						...
						<span
							className={`flex items-center gap-1.5 px-2.5 py-1 text-[10px] sm:text-xs font-regular rounded-full ${
								job.type === "Export"
									? "bg-[#C3BEF0] text-[#3525CD]"
			...
			<div className="bg-white rounded-xl p-4 sm:p-6 shadow-sm border border-gray-100 overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
				<h2 className="text-md font-semibold text-[#000000] mb-6 sm:mb-8">
					Status Timeline
				</h2>
				<div className="relative flex flex-row items-start justify-between min-w-[800px] w-full mx-auto">
					...
					<TimelineStep
						title={job.status === "In Transit" ? "In Transit" : job.status}
						date="Current Status"
						active
						current
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
				...
				<div className="lg:col-span-2 bg-white rounded-xl p-4 sm:p-6 shadow-sm border border-gray-100 grid grid-cols-1 xl:grid-cols-2 gap-4 sm:gap-6">
					...
					<div className="bg-[#F4F4F4] rounded-xl p-4 sm:p-6 flex flex-col border border-gray-100">
						<div className="flex justify-between items-center mb-4 sm:mb-6">
							<h2 className="text-[16px] sm:text-[18px] font-semibold text-[#000000]">
								Shipment Overview
							</h2>
						...
						<div className="space-y-4 sm:space-y-6 flex-1">
							...
							<div className="pt-2">
								...
								<div className="font-bold text-gray-900 text-sm flex flex-wrap items-center gap-2">
									{details.routePlanOrigin}{" "}
									<ArrowRight className="w-3 h-3 text-gray-400 shrink-0" />{" "}
									{details.routePlanDestination}
								</div>
							</div>
						</div>
					</div>
				...
				<div className="lg:col-span-2 bg-white rounded-xl p-4 sm:p-6 shadow-sm border border-gray-100 flex flex-col overflow-hidden">
					<h2 className="text-[16px] sm:text-[18px] font-semibold text-[#000000] mb-4 sm:mb-6">
					...
					<div className="overflow-x-auto flex-1 w-full">
						<table className="w-full text-sm text-left min-w-[500px]">
							<thead className="bg-[#F4F4F4] text-[#000000CC] font-medium border-b border-gray-100">
								<tr>
									<th className="py-3 px-4 font-medium rounded-tl-lg whitespace-nowrap">
										Container Type
									</th>
									...
									<th className="py-3 px-4 font-medium rounded-tr-lg whitespace-nowrap">
										HS Codes
									</th>
								</tr>
							</thead>
							<tbody className="divide-y divide-gray-100">
								{details.containers.map((c, i) => (
									<tr
										key={i}
										className="even:bg-[#F8F8F8] text-[#000000] hover:bg-gray-50 transition-colors"
									>
										<td className="py-4 px-4 font-medium whitespace-nowrap">
											{c.type}
										</td>
										<td className="py-4 px-4 font-medium whitespace-nowrap">
											{c.quantity < 10 ? `0${c.quantity}` : c.quantity}
										</td>
										<td className="py-4 px-4 font-medium whitespace-nowrap">
											{c.grossWeight}
										</td>
				...
				<div className="bg-white rounded-xl p-4 sm:p-6 shadow-sm border border-gray-100 flex flex-col">
					<div className="flex justify-between items-center mb-4 sm:mb-6">
						...
						<span className="px-2 py-0.5 bg-[#ACF1AD] text-[#249209] text-[10px] font-semibold rounded">
							Active
						</span>
					</div>
					<div className="space-y-4 flex-1">
						...
						<div>
							<div className="text-[10px] text-gray-500 uppercase tracking-wider mb-1">
								{details.insuranceCoverage.toUpperCase()}
							</div>
							<div className="font-bold text-gray-900 text-sm">
					...
					<div className="space-y-4 flex-1">
						{details.internalNotes.map((note, i) => (
							<div key={i} className="bg-[#F2F2F2] rounded-lg p-4">
								<p className="text-sm text-[#000000] font-medium mb-2">
									{note.text}
								</p>
								<p className="text-xs text-[#000000B2] font-regular">
					...
					<div className="space-y-2 relative">
						{details.activityLogs.map((log, i) => (
							<div key={i} className="relative flex items-start gap-4">
								...
								<div>
									<div className="text-md font-medium text-[#000000]">
										{log.title}
									</div>
									<div className="text-sm text-[#000000]/70 font-regular mt-1">
										{log.subtitle}
									</div>
								</div>
							</div>
...
function TimelineStep({
	...
	return (
		<div className="relative flex flex-col items-center flex-1">
			...
			{!isLast && (
				<div
					className={`absolute top-[86px] left-[50%] w-full h-[2px] flex items-center z-0 ${active && !current ? "bg-[#E5E7EB]" : "bg-[#E5E7EB]"}`}
				>
					<div className="mx-auto bg-white px-1.5">
						...
						>
							<path
								...
								fill={active && !current ? "#04458B" : "#D1D5DB"}
							/>
						</svg>
					</div>
				</div>
			...
			{/* Icon Squircle */}
			<div
				className={`relative z-10 w-[56px] h-[56px] rounded-[18px] flex items-center justify-center text-white mb-6 ${
					active || current
						? "bg-linear-to-b from-[#075EB6] to-[#04478E]"
						: "bg-[#64748B]"
				}`}
			>
			...
			{/* Small Dot */}
			<div
				className={`relative z-10 w-[14px] h-[14px] rounded-full mb-3 outline outline-[3px] outline-white ${
			...
			<div className="text-center px-2">
				<div
					className={`text-xs font-medium ${current ? "text-[#000000]" : "text-[#000000]"}`}
				>
					{title}
				</div>
				<div
					className={`text-[10px] mt-1 ${current ? "text-[#04458B] font-regular" : "text-[#000000]"}`}
				>
					{date}
				</div>
			</div>
	...
	return (
		<div className="flex items-center justify-between group border-b border-gray-300 pb-4 last:border-0 last:pb-0">
			<div className="flex items-center gap-3 overflow-hidden">
				<div
					className={`p-2 rounded shrink-0 ${type === "pdf-success" ? "bg-[#248700]/10" : "bg-[#FF0000]/10"}`}
				>
					{type === "pdf-success" ? (
			...
			<div className="flex items-center gap-2 sm:gap-3 shrink-0">
				<span
					className={`px-2 py-0.5 text-[10px] font-semibold rounded ${
						status === "Confirmed"
							? "bg-[#ACF1AD7D] text-[#249209]"
							: "bg-[#FFDA9C9E] text-[#D98C0B]"
					}`}
				>
					{status}
				</span>
				<div className="font-medium text-[#000000] w-16 sm:w-20 text-right">
					{amount}
				</div>
