...
function FileIcon({ ext }: { ext: string }) {
	...
	if (ext === "jpg" || ext === "png")
		return <FileImage className="w-5 h-5 text-[#1E8449]" />;
	if (ext === "docx" || ext === "doc")
		return <FileText className="w-5 h-5 text-[#1A73E8]" />;
	if (ext === "xlsx")
		return <FileSpreadsheet className="w-5 h-5 text-[#1E8449]" />;
	return <FileArchive className="w-5 h-5 text-gray-400" />;
}

export function CustomerDocuments() {
	const [activeCategory, setActiveCategory] =
		useState<DocCategory>("All Documents");
		...
		<div className="flex flex-col gap-6">
			...
			<div className="bg-white border border-gray-200 rounded-[20px] p-6">
				...
				<div className="w-full bg-gray-100 rounded-full h-2.5 mb-2">
					<div
						className="bg-[#1E8449] h-2.5 rounded-full transition-all"
						style={{ width: "98%" }}
					></div>
				</div>
							...
							<tbody className="divide-y divide-gray-50">
								{filtered.map((doc, idx) => (
									<tr key={idx} className="hover:bg-gray-50/50 transition">
										...
										<td className="py-4 px-4">
											<span
												className={`text-[12px] font-semibold px-2.5 py-1 rounded-full ${doc.typeClass}`}
											>
												{doc.type}
											</span>
										</td>
					...
					<div className="flex flex-col gap-2">
						{categories.map((cat) => (
							...
							>
								...
								<span
									className={`text-[13px] font-bold ${activeCategory === cat.label ? "text-primary" : "text-white/70"}`}
								>
									{cat.count}
								</span>
							</button>
