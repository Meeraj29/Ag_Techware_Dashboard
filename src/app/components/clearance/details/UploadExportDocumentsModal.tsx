...
export default function UploadExportDocumentsModal({
	isOpen,
	onClose,
	jobId,
}: UploadExportDocumentsModalProps) {
	...
	const handleUpload = (id: number) => {
		...
		setDocuments((docs) =>
			docs.map((doc) =>
				doc.id === id
					? { ...doc, status: "Uploaded", date: "June 16, 2026" }
					: doc,
			),
		);
	};
					...
					<table className="w-full text-left">
						<thead>
							<tr className="bg-[#F3F4F6]">
								<th className="py-4 px-6 text-sm font-medium text-gray-700">
									Document Type
								</th>
								<th className="py-4 px-6 text-sm font-medium text-gray-700">
									Status
							...
							{documents.map((doc, idx) => (
								...
								>
									...
									<td className="py-4 px-6">
										<span
											className={`text-sm font-medium ${doc.status === "Uploaded" ? "text-[#027A48]" : "text-[#D92D20]"}`}
										>
											{doc.status}
										</span>
									</td>
