...
export default function CreateJobPage() {
	...
	const [containers, setContainers] = useState([
		{ id: "1", type: "", quantity: "", weight: "", unit: "KG" },
	]);
	...
	const handleSaveDetails = () => {
		...
		const valueAddServicesList = Object.keys(services)
			.filter((k) => services[k])
			.map((k, i) => ({
				id: k,
				name: k
		...
		dispatch(
			addJob({
				...
				category: "Export",
				route: {
					origin: routeDetails.pol || "-",
					destination: routeDetails.pod || "-",
					type: basicDetails.freightType || "Standard",
				},
				status: "Job Created",
